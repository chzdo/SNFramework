...
const writeExcelSheet = async function ({
  title = "main",
  rows,
  columns,
  name,
...
const format = {
  date: value => {
    ...
    return (0, _dayjs.default)(value).toISOString().split("T")[0];
  }
};
const toExcelFile = async function ({
  title,
  sheets,
  fileName,
  stream
...
const toTextFile = async ({
  ...
  if (isJson) {
    await _fsExtra.default.writeJSON(`./${title}.${reportType}`, rows);
    return;
  }
...
const toPdfFile = async function ({
  fileName,
  settings,
  stream
}) {
  ...
  await _htmlPdfNode.default.generatePdf({
    content: tableHTML
  }, pdfOptions).then(pdfBuffer => {
    if (stream) {
      const readable = new _readableStream.Readable();
      ...
      _fsExtra.default.writeFile(fileName, pdfBuffer);
    }
  }).catch(error => {
